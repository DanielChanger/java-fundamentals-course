...
public class CrazyGenerics {
     ...
     * @param <T> – value type
     */
    public static class MaxHolder<T extends Comparable<T> & Serializable> {
        private T max;
     ...
     * @param <C> – a type of entities collection
     */
    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            return entities.stream()
                    .max(Comparator.comparing(T::getCreatedOn))
...
//            Collections.swap(elements, i, j);
            swapParametrized(elements, i, j);
        }

        private static <T> void swapParametrized(List<T> elements, int i, int j) {
